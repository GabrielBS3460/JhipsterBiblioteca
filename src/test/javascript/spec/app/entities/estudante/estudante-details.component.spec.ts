/* tslint:disable max-line-length */
import { shallowMount, createLocalVue, Wrapper } from '@vue/test-utils';
import sinon, { SinonStubbedInstance } from 'sinon';
import VueRouter from 'vue-router';

import * as config from '@/shared/config/config';
import EstudanteDetailComponent from '@/entities/estudante/estudante-details.vue';
import EstudanteClass from '@/entities/estudante/estudante-details.component';
import EstudanteService from '@/entities/estudante/estudante.service';
import router from '@/router';
import AlertService from '@/shared/alert/alert.service';

const localVue = createLocalVue();
localVue.use(VueRouter);

config.initVueApp(localVue);
const i18n = config.initI18N(localVue);
const store = config.initVueXStore(localVue);
localVue.component('font-awesome-icon', {});
localVue.component('router-link', {});

describe('Component Tests', () => {
  describe('Estudante Management Detail Component', () => {
    let wrapper: Wrapper<EstudanteClass>;
    let comp: EstudanteClass;
    let estudanteServiceStub: SinonStubbedInstance<EstudanteService>;

    beforeEach(() => {
      estudanteServiceStub = sinon.createStubInstance<EstudanteService>(EstudanteService);

      wrapper = shallowMount<EstudanteClass>(EstudanteDetailComponent, {
        store,
        i18n,
        localVue,
        router,
        provide: { estudanteService: () => estudanteServiceStub, alertService: () => new AlertService() },
      });
      comp = wrapper.vm;
    });

    describe('OnInit', () => {
      it('Should call load all on init', async () => {
        // GIVEN
        const foundEstudante = { id: 123 };
        estudanteServiceStub.find.resolves(foundEstudante);

        // WHEN
        comp.retrieveEstudante(123);
        await comp.$nextTick();

        // THEN
        expect(comp.estudante).toBe(foundEstudante);
      });
    });

    describe('Before route enter', () => {
      it('Should retrieve data', async () => {
        // GIVEN
        const foundEstudante = { id: 123 };
        estudanteServiceStub.find.resolves(foundEstudante);

        // WHEN
        comp.beforeRouteEnter({ params: { estudanteId: 123 } }, null, cb => cb(comp));
        await comp.$nextTick();

        // THEN
        expect(comp.estudante).toBe(foundEstudante);
      });
    });

    describe('Previous state', () => {
      it('Should go previous state', async () => {
        comp.previousState();
        await comp.$nextTick();

        expect(comp.$router.currentRoute.fullPath).toContain('/');
      });
    });
  });
});
